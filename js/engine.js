...
(function (root, factory) {
  ...
  function legalActions(s) {
    ...
    if (s.megasEnabled && p.megaToken < 1 && s.supply.megaToken > 0) acts.push({ type: 'takeMega' });
    ...
    for (const id of capIds) { const action = defaultCaptureAction(s, p, id); if (action) acts.push(action); }
  ...
  return {
    ...
    PM_TIERS, PM_SLOTS, fieldTiers, isPokemart, effBonusColor, freeTiers, freeTakeable, CANON_SPECIAL,
